...
export default function App() {
  ...
  const [createTableInitialStake, setCreateTableInitialStake] = useState<number>(0);
  const [isChallengeModalOpen, setIsChallengeModalOpen] = useState(false);
  const [challengeModalInitialStake, setChallengeModalInitialStake] = useState<number>(0);
    ...
    <div className="h-screen max-h-screen bg-slate-950 text-slate-100 font-sans flex flex-col selection:bg-amber-500 selection:text-slate-950 overflow-hidden">
      ...
      <main className="flex-1 overflow-hidden min-h-0">
        {activeRoom ? (
          ...
          <OnlineLobby
            ...
            gameRooms={gameRooms}
            onInitiateChallenge={(targetUser, preselectedStake) => {
              setChallengeTargetPlayer(targetUser);
              if (preselectedStake !== undefined) {
                setChallengeModalInitialStake(preselectedStake);
              } else {
                setChallengeModalInitialStake(0);
              }
              setIsChallengeModalOpen(true);
            }}
            onCreateCustomGame={handleCreateCustomGame}
            onOpenCreateTableModal={(preselectedStake?: number) => {
              setCreateTableInitialStake(preselectedStake || 0);
      ...
      {currentUser && (
        <CreateTableModal
          ...
          initialStake={createTableInitialStake}
      ...
      {currentUser && (
        <ChallengeModal
          ...
          initialStake={challengeModalInitialStake}
          onClose={() => {
