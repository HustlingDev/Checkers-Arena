...
// Enable CORS for native mobile apps, Capacitor WebView, and cross-origin clients
...
// ==========================================================
// PesaJet Mobile Money API Integration (MTN MoMo & Airtel Money)
// ==========================================================
// PesaJet Configuration & Status Check API
...
// PesaJet Transaction Status Verification API
app.get(['/api/pesajet/verify-status', '/api/payments/verify-status'], async (req, res) => {
  try {
    ...
    // Find pending transaction in memory
...
app.all(['/api/pesajet/webhook', '/api/pesajet/ipn'], async (req, res) => {
  try {
    ...
    const signature = req.headers['x-pesajet-signature'] as string | undefined;
...
// PesaJet Webhook Info Endpoint (Assists user with destination URL configuration)
...
// Wallet Withdrawal Request API (Disburse via PesaJet Mobile Money)
